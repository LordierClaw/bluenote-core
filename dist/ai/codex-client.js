...
function getString(input, field) {
...
function normalizeSseResponse(text) {
    const outputDeltas = [];
    let outputDone;
...
async function readCodexResponseBody(response) {
...
function normalizeResponse(body, response, providerRequestId) {
    if (!isRecord(body)) {
...
function isNearExpired(auth, now) {
...
export function createCodexTextGenerationClient(options) {
    return {
        async createChatCompletion(request) {
            ...
            if (!response.ok) {
                ...
                throw new CodexTextGenerationClientError(`Codex provider request failed with status ${response.status}${context}: ${details}.`);
            }
