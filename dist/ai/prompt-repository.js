...
import { getAiPromptsPath } from "../storage/root-layout.js";
...
function buildDefaultDescribeNotePrompt(outputLanguage = DEFAULT_OUTPUT_LANGUAGE) {
...
const LEGACY_DEFAULT_DESCRIBE_NOTE_PROMPTS = new Set([
...
export function hashAiPromptContent(content) {
    return `sha256:${createHash("sha256").update(content, "utf8").digest("hex")}`;
}
function getDescribeNotePromptPath(rootPath) {
    return path.join(getAiPromptsPath(rootPath), DESCRIBE_NOTE_PROMPT_FILENAME);
}
function relativePromptPath(rootPath, promptPath) {
    return path.relative(path.resolve(rootPath), promptPath) || promptPath;
}
function toPrompt(promptPath, content, relativePath) {
...
function defaultPromptForRoot(rootPath) {
    try {
...
export function ensureDescribeNotePrompt(rootPath) {
...
export function readDescribeNotePrompt(rootPath) {
