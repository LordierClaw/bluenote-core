...
import { getAiQueuePath } from "../storage/root-layout.js";
import { toPortableRelativePath } from "../platform/path-safety.js";
const EMPTY_QUEUE = { version: 1, jobs: [] };
const LOCK_STALE_AFTER_MS = 10 * 60 * 1000;
function getQueueLockMetadataPath(lockPath) {
...
function readQueueLockMetadata(lockPath) {
    try {
        const parsed = JSON.parse(readFileSync(getQueueLockMetadataPath(lockPath), "utf8"));
...
function isStaleQueueLock(lockPath, now = Date.now()) {
...
function acquireQueueLock(lockPath, relativePath) {
    try {
...
function relativeQueuePath(rootPath, queuePath) {
    return toPortableRelativePath(path.relative(rootPath, queuePath) || queuePath);
}
function assertPlainObject(input, sourcePath) {
...
function assertString(value, field, sourcePath) {
...
function assertNullableString(value, field, sourcePath) {
    if (value === null) {
        return null;
    }
    return assertString(value, field, sourcePath);
}
function assertSha256(value, field, sourcePath) {
...
function assertIsoOrNull(value, field, sourcePath) {
    if (value === null) {
        return null;
    }
...
function assertIso(value, field, sourcePath) {
    return assertIsoOrNull(value, field, sourcePath) ?? assertString(value, field, sourcePath);
}
function validateDescribeNoteJob(input, sourcePath) {
...
export function validateAiQueue(input, sourcePath) {
    assertPlainObject(input, sourcePath);
    if (input.version !== 1) {
...
export function createAiQueueRepository(rootPath) {
