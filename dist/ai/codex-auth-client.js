...
function requireString(body, field, context) {
...
function readIntervalSeconds(body) {
...
function classifyDeviceError(status, body) {
...
function sanitizeForError(error, secrets) {
...
export function createCodexAuthClient(options = {}) {
    ...
    const secretsForFlow = (flow) => [flow.deviceAuthId, flow.userCode];
    async function request(input, init, signal) {
        try {
    ...
    async function startDeviceFlow(startOptions = {}) {
        ...
        if (!response.ok) {
            ...
            throw new CodexAuthClientError("http", `Codex device auth start failed with status ${response.status}: ${sanitizeForError(providerErrorText(body) ?? response.statusText, [])}.`);
        }
    ...
    async function pollDeviceFlow(flow, pollOptions = {}) {
        ...
        while (true) {
            ...
            throw new CodexAuthClientError(code, `Codex device auth failed with status ${response.status}: ${sanitizeForError(providerErrorText(body) ?? response.statusText, secrets)}.`);
        }
    }
    async function exchangeAuthorizationCode(code, exchangeOptions = {}) {
        ...
        if (!response.ok) {
            throw new CodexAuthClientError("http", `Codex token exchange failed with status ${response.status}: ${sanitizeForError(providerErrorText(body) ?? response.statusText, secrets)}.`);
        }
        return buildAuthFromTokenResponse({ body, now: now(), issuer, clientId });
    }
    async function completeDeviceFlow(flow, completeOptions = {}) {
    ...
    return {
        ...
        async refreshAuth(auth, refreshOptions = {}) {
            ...
            if (!response.ok) {
                throw new CodexAuthClientError("setup-required", `Codex token refresh failed with status ${response.status}: ${sanitizeForError(providerErrorText(body) ?? response.statusText, secrets)}. Run bn ai codex auth login.`);
            }
            return buildAuthFromTokenResponse({ body, now: now(), issuer: auth.issuer || issuer, clientId: auth.clientId || clientId, existing: auth });
        },
    };
}
//# sourceMappingURL=codex-auth-client.js.map