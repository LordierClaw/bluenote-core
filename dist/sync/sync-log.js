...
function isRedactedFieldName(key) {
...
function isRedactedQueryKey(key) {
    const normalized = normalizeFieldName(key);
...
function redactParams(rawValue) {
...
function redactFragment(rawValue) {
...
function redactString(value) {
    ...
    const withEmbeddedParams = withEmbeddedUrls.replace(/([?#])([^\s"'<>]+)/gu, (match, prefix, rawParams) => {
        const params = redactParams(rawParams);
        return params.changed ? `${prefix}${params.value}` : match;
    });
    const withEmbeddedBareParams = withEmbeddedParams.replace(/(^|\s)([^\s"'<>?#=&]+=[^\s"'<>?#&]+(?:&[^\s"'<>?#=&]+=[^\s"'<>?#&]+)+)/gu, (match, prefix, rawParams) => {
        const params = redactParams(rawParams);
        return params.changed ? `${prefix}${params.value}` : match;
    });
    const withEmbeddedAssignments = withEmbeddedBareParams.replace(/(^|\s)([^\s"'<>?#=&]+)=([^\s"'<>?#&]+)/gu, (match, prefix, key, rawValue) => {
        if (!isRedactedQueryKey(key)) {
            return match;
        }
...
export function redactSyncLogValue(value) {
...
export function createSyncLogWriter(options) {
