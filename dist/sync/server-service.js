...
import { getArchiveNotesPath, getDraftNotesPath, getNormalNotesPath } from "../storage/root-layout.js";
...
function findNoteIdByKey(rootPath, key) {
    const stateNotesPath = assertPathInsideRoot(rootPath, path.join(rootPath, STATE_NOTES_DIRECTORY));
    if (!fs.existsSync(stateNotesPath)) {
        return null;
    }
    for (const entry of fs.readdirSync(stateNotesPath, { withFileTypes: true })) {
        ...
        try {
            const parsed = JSON.parse(fs.readFileSync(path.join(stateNotesPath, entry.name), "utf8"));
            if (typeof parsed === "object" && parsed !== null && parsed.key === key) {
                const noteId = parsed.noteId;
                return typeof noteId === "string" ? noteId : path.basename(entry.name, ".json");
            }
        }
...
function assertNoteKeyAvailable(rootPath, key, ownerNoteId) {
...
function applyFolderPush(rootPath, handle, record) {
    ...
    const previousFolderRows = handle.db.exec("SELECT relativePath, createdAt, updatedAt, deletedAt FROM folders WHERE relativePath = ?", [relativePath])[0]?.values ?? [];
    assertPathAndParentsAreNotSymlinks(rootPath, folderPath);
    const rollback = () => {
        ...
        try {
            ...
            for (const row of previousFolderRows) {
                handle.db.run("INSERT INTO folders (relativePath, createdAt, updatedAt, deletedAt) VALUES (?, ?, ?, ?)", row);
            }
        }
        catch {
            // Best-effort rollback: preserve original sync error.
...
function readAiPublishSnapshot(rootPath, noteId) {
    const sidecars = createSidecarRepository(rootPath);
...
function publishAiSidecarChange(rootPath, dbIdentity, workspaceId, noteId) {
...
function queueAiWork(rootPath, dbIdentity, workspaceId, queueAiWork, noteId) {
