...
function findSuggestedKeys(selector, notes) {
...
function assertSingleMatch(selector, matches) {
    if (matches.length === 1) {
        return matches[0];
    }
    throw new AmbiguousSelectorError(`Ambiguous note selector: ${selector}. Matches: ${matches.map((note) => note.sourcePath).join(", ")}.`, {
        hint: "Use a note key or managed-root-relative path to disambiguate.",
    });
}
export function selectNote(options) {
...
//# sourceMappingURL=select-note.js.map