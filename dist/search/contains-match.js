...
export function containsSearchQuery(value, query) {
...
export function scoreContainsMatch(value, query, weight = 1) {
    const normalizedValue = normalizeSearchQuery(value);
    const normalizedQuery = normalizeSearchQuery(query);
    if (normalizedValue.length === 0 || normalizedQuery.length === 0) {
        return 0;
...
export function collectContainsFieldMatches(query, candidates) {
    const matches = [];
...
function getContainsMatchKind(normalizedValue, normalizedQuery) {
...
function getBaseScore(matchKind) {
    if (matchKind === "exact") {
...
function getRatioBonus(normalizedValue, normalizedQuery) {
    return Math.min(10, Math.round((normalizedQuery.length / normalizedValue.length) * 10));
}
function hasContiguousTokenMatch(normalizedValue, normalizedQuery) {
    ...
    for (let index = 0; index <= valueTokens.length - queryTokens.length; index += 1) {
        const windowMatches = queryTokens.every((queryToken, tokenIndex) => valueTokens[index + tokenIndex] === queryToken);
        if (windowMatches) {
            return true;
        }
    }
...
//# sourceMappingURL=contains-match.js.map